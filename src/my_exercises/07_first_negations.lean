...
begin
  split,
  { intros notP, cases h₁ with hP hQ,
    { exfalso, apply notP, exact hP, },
    { exact hQ, }
  },
  { intros hQ hP, apply h₂, exact ⟨hP, hQ⟩, }
end
...
begin
  ...
  { intro h,
    by_cases hP : P,
    { right, intros hQ, apply h, exact ⟨hP, hQ⟩, },
    { left, exact hP, }
  },
  { rintros h ⟨hP, hQ⟩,
    cases h with h h,
...
begin
  ...
  { rintros h ⟨k, rfl⟩, apply h k, refl, }
end
...
begin
  ...
  { intros h x y, contrapose!, rw ← h, intro hle, exact hle, }
end
