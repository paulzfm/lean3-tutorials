...
begin
  ...
  split, 
  { apply le_max_right, },
  { calc n ≤ max n n'      : by apply le_max_left
...
begin
  intros hcp ε hε n,
  rcases hcp with ⟨φ, hφ, ha⟩,
  cases ha ε hε with N ha,
  rcases extraction_ge hφ n N with ⟨m, hm, hφ⟩,
  exact ⟨φ m, hφ, ha m hm⟩,
end
...
begin
  rcases ha with ⟨φ, hφ, h₁⟩,
  have h₂ : seq_limit (u ∘ φ) l, from subseq_tendsto_of_tendsto' hl hφ,
  exact unique_limit h₁ h₂,
end
...
begin
  ...
  rcases near_cluster hl (ε/2) (by linarith) N with ⟨m, hm, hl⟩,
  use N, intros n hn,
  calc
  |u n - l| = |(u n - u m) + (u m - l)|   :   by ring
        ... ≤ |u n - u m| + |u m - l|     :   by apply abs_add
        ... ≤ ε                           :   by linarith [hN n m hn hm]
end
