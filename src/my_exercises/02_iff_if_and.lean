...
begin
  rw ← sub_nonneg,
  have h : a * c - b * c = (a - b) * c, { by ring },
  rw h,
  apply mul_nonneg_of_nonpos_of_nonpos,
  { rw sub_nonpos, exact hab },
  { exact hc },
end
...
example (a b c : ℝ) (hc : c ≤ 0) (hab :  a ≤ b) : b*c ≤ a*c :=
begin
  have h₁ : a - b ≤ 0, { rw sub_nonpos, exact hab },
  have h₂ : 0 ≤ (a - b) * c, from mul_nonneg_of_nonpos_of_nonpos h₁ hc,
...
begin
  ...
  { intros hab,
    apply dvd_antisymm,
    { exact h.1, },
    { rw dvd_gcd_iff, exact ⟨dvd_refl a, hab⟩, }
  },
  { intros hab,
    rw ← hab,
    exact h.2,
